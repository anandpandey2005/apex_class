...
import { User, Phone, Menu, LogOut } from 'lucide-react';
...
export const StudentHeader: React.FC<StudentHeaderProps> = ({ onToggleMobileMenu }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      dispatch(logoutUser());
      dispatch(showToast({ message: 'Logged out of student portal.', type: 'info' }));
  ...
  return (
    <header className="h-16 border-b border-zinc-800 bg-zinc-950 px-4 md:px-6 flex items-center justify-between sticky top-0 z-10">
      <div className="flex items-center space-x-3">
        ...
        <h3 className="text-xs md:text-sm font-bold text-white mb-0 tracking-tight truncate max-w-[150px] sm:max-w-none">
          {instituteName}
        </h3>
